...
abstract class WebSocketServer {
  ...
  function __construct($addr, $port, $bufferLength = 2048) {
    ...
    socket_listen($this->master,100000)                               or die("Failed: socket_listen()");
    $this->sockets['m'] = $this->master;
    $this->stdout("Сервер запущен\nПрослушивается по адресу: $addr:$port\nМастер-сокет: ".$this->master);
   ...
   */
  public function run() {//№1 - Запускается при обращении и создает сервер
    set_time_limit(0);
    while(true) {
     ...
     // $this->_tick();
      //$this->tick();
      @socket_select($read,$write,$except,1);//ожидает изменения состояния на сокетах в массиве $read
      foreach ($read as $socket) {//№2 - Перебирает все соккеты (в масс. рид остались измененн. сокеты)
        if ($socket == $this->master) {//№3 - Изменился ли главный сокет (новое подключение?)
          $client = socket_accept($socket);// создаем новый сок ресурс
          if ($client < 0) {//если ошибка
            $this->stderr("Failed: socket_accept()");
            continue;
          } 
          else {//если успешно создали...
            $this->connect($client);//подключаем клиента, возвращаемся к началу цикла, теперь проверим
            //на рукопожатие, если знакомы обработаем сообщение
            $this->stdout("Клиент подключен. " . $client);
          }
        } 
        else {
          $numBytes = @socket_recv($socket, $buffer, $this->maxBufferSize, 0); //Читаем данные из сокета
          if ($numBytes === false) { // Ошибка соединения
          ...
          elseif ($numBytes == 0) { //Клиент отключился
            $this->disconnect($socket);
            $this->stderr("Клиент отключен. TCP соединение потеряно: " . $socket);
          } 
          else {
            $user = $this->getUserBySocket($socket);
            if (!$user->handshake) {// еслин не знаком
              $tmp = str_replace("\r", '', $buffer);
              if (strpos($tmp, "\n\n") === false ) {
                continue; // If the client has not finished sending the header, then wait before sending our upgrade response.
              }
              $this->doHandshake($user,$buffer);// №4 - делаем рукопожатие
             ...
             foreach ($this->dialogs as $dialog => $users_arr){
              if($dialog == $user->dialog){
                ...
                foreach($users_arr as $send_to_user){
                  $this->split_packet($numBytes,$buffer, $send_to_user);
                }
              }
            }    
          }
...
protected function connect($socket) {
  $user = new $this->userClass(uniqid('u'), $socket);//Создаем нового юзера, с ид и сокетом
  $this->users[$user->id] = $user;//добавляем в массив всех юзеров
  $this->sockets[$user->id] = $socket;//его сокет добавляем в массив всех сокетов
...
protected function doHandshake($user, $buffer) {
  ...
  $user->get = str_replace('/?', '', $headers['get']);
  $ids = explode(',', $user->get);
  $user->from = (int)$ids[0];//19
  $user->to = (int)$ids[1];//21
  $user->dialog = (int)$ids[0]+(int)$ids[1]; //(19+21) = 40
  if(isset($this->dialogs[$user->dialog])){
    array_push($this->dialogs[$user->dialog], $user);
  }else{
    $this->dialogs[$user->dialog] = [];
    array_push($this->dialogs[$user->dialog], $user);
  }
